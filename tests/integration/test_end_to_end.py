...
def test_end_to_end_train_control():
    ...
    command_response = requests.post(f"{BASE_URL}/{train_id}/command", json=command_payload)
    ...
    stop_command_response = requests.post(f"{BASE_URL}/{train_id}/command", json=stop_command_payload)
