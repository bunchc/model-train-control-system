...
def get_controller_id():
    ...
    Checks environment variable CONTROLLER_ID first, then config file.
...
def get_central_api_url(service_config):
    ...
    Returns:
        str: Base URL for central API (e.g. 'http://central_api:8000').
    """
...
def download_runtime_config(uuid, service_config):
    ...
    Saves to edge-controller.yaml, preserving the UUID.

    Args:
        uuid (str): Controller UUID.
        service_config (dict): Service config dict.

    Returns:
        bool: True if download succeeded, False otherwise.
    Side Effects:
        Performs HTTP GET request and writes to disk.
    """
    try:
        base_url = get_central_api_url(service_config)
        ...
        logging.error(f"Failed to download runtime config: {e}")
    return False
...
def register_with_central_api(service_config):
    ...
    Stores the UUID in edge-controller.yaml.
    ...
    Side Effects:
        Performs HTTP POST request and writes to disk.
    """
    import socket
    try:
        ...
        if resp.status_code == 200:
            ...
            uuid = data.get('uuid')
            if uuid:
                logging.info(f"Registered controller name={hostname}, address={ip_address}, uuid={uuid}, status={data.get('status')}")
                return uuid
    except Exception as e:
...
def initialize_configs():
    ...
    Handles central API and controller registration, config download, and fallback to cached config.
    ...
    Side Effects:
        May perform HTTP requests, read/write disk, and log errors.
    """
    ...
    if not is_central_api_accessible(service_config):
        ...
        if runtime_config:
            ...
            logging.error("Central API is not accessible and no cached runtime config. Failing to start.")
    ...
    if cached_config and 'uuid' in cached_config:
        ...
        if download_runtime_config(uuid, service_config):
            ...
            if 'train_id' in cached_config and 'mqtt_broker' in cached_config:
                ...
                logging.warning("Failed to download config and cached config is incomplete. Will run without train config.")
    ...
    if download_runtime_config(uuid, service_config):
        ...
        logging.info("Registered successfully but no runtime config available yet. Starting without train config.")
